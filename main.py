...
class Network:
        ...
        if command.strip() == ('list'):
            ...
            for i in self.node_list:
                if int(i.id_) == id_:
                    ...
                    if reply == "freezed":
                        print(
                            f"Freezing Node with ID {i.id_} on PORT:{i.port}")
            ...
            for i in self.suspended_list:
                if int(i.id_) == id_:
                    ...
                    if reply == "unfreezed":
                        print(
                            f"Unfreezing Node with ID {i.id_} on PORT:{i.port}")
            ...
            file = None
            self.coordinator.is_coordinator = False
            try:
    ...
    def reload(self, filename):
        ...
        send_message("undo coordinator", self.coordinator.port)
        self.coordinator = self.startElection(default=True)
        send_message("make coordinator", self.coordinator.port)
        self.sync_clocks()
        time.sleep(1)
